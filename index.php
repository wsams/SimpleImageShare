...
function mogrify($img) {
    exec("{$GLOBALS['mogrify']} -resize 256x256 \"$img\"");
    exec("{$GLOBALS['mogrify']} -quality 75 \"$img\"");
}
...
if ($_GET['action'] == "upload") {
    if (is_array($_FILES) && count($_FILES) > 0) {
        ...
        $newname = date("U") . getmicrotime() . $_FILES['file']['name'];
        ...
        if (preg_match("/\.zip$/i", $name)) {
            $imghtml .= "<div class='success'>Success, zip loaded just fine.</div>";
            ...
            foreach ($a_list as $k=>$img) {
                ...
                copy($tmpzipdir . "/" . $newname . "/" . $oldfilepath, $newfilepaththumb);
                mogrify($newfilepaththumb);
            }
            ...
            copy($imgdir . "/" . $newname, $imgdir . "/thumb_" . $newname);
            mogrify($imgdir . "/thumb_" . $newname);
        }
...
if (is_array($imgs) && count($imgs) > 0) {
    foreach ($imgs as $k=>$v) {
        ...
        $imghtml .= "<a target=\"_blank\" href=\"{$imgdir}/{$realimg}\"><img src=\"{$imgdir}/{$v}\" alt=\"{$imgdir}/{$v}\" /></a> ";
    }
}
